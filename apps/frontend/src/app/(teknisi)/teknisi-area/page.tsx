...
import {
  ...
  Zap, LifeBuoy, TrendingUp, ArrowRight
} from 'lucide-react';
...
export default function TeknisiDashboard() {
  ...
  return (
    <div className="relative min-h-screen pb-20 z-0">
      ...
      {(() => {
        const desktopSubtitle = highPriorityOpen.length > 0
          ? `Ada ${highPriorityOpen.length} tiket prioritas tinggi yang perlu segera ditangani.`
          : myInProgressTickets.length > 0
            ? `Kamu sedang menangani ${myInProgressTickets.length} tiket aktif. Tetap semangat!`
            : 'Semua tiket terkendali. Pantau antrean secara rutin.';

        return (
          <div className="mb-4 md:mb-8 pt-4 md:pt-0 flex items-start justify-between">
            <div>
              {/* Mobile: first name only | Desktop: full name */}
              <h1 className="text-xl font-black text-white tracking-tight md:hidden">
                Halo, {user?.name?.split(' ')[0] || 'Teknisi'}
              </h1>
              <h1 className="hidden md:block text-2xl font-black text-slate-800 tracking-tight">
                Selamat datang, {user?.name || 'Teknisi'}
              </h1>
              {/* Mobile subtitle */}
              <p className="text-xs font-medium text-emerald-100 mt-1 md:hidden">Siap menyelesaikan masalah IT hari ini?</p>
              {/* Desktop dynamic subtitle */}
              <p className={`hidden md:block text-sm font-semibold mt-1 ${highPriorityOpen.length > 0 ? 'text-red-500' : 'text-slate-500'
                }`}>
                {desktopSubtitle}
              </p>
            </div>
      ...
      <div className="md:flex md:gap-6 md:items-start">
        ...
        <div className="hidden md:flex flex-col gap-5 w-[290px] flex-shrink-0">
          ...
          {/* --- Widget 3: Aktivitas Terkini (Rich Feed) --- */}
          <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <Activity className="w-4 h-4 text-slate-500" />
                <h3 className="text-xs font-extrabold text-slate-700 uppercase tracking-wider">Aktivitas Terkini</h3>
              </div>
              <button
                onClick={() => setIsNotifOpen(true)}
                className="relative text-[10px] font-extrabold text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-full hover:bg-emerald-100 flex items-center gap-1"
              >
                <Bell className="w-3 h-3" />
                Notifikasi
                {unreadCount > 0 && <span className="w-1.5 h-1.5 rounded-full bg-red-500 inline-block ml-0.5" />}
              </button>
            </div>
            <div className="space-y-3">
              {latestActivity.length === 0 ? (
                ...
                latestActivity.map(ev => (
                  <Link key={ev.id} href={`/teknisi-area/helpdesk/${ev.ticketId}`} className="flex items-start gap-3 group hover:bg-slate-50 rounded-xl p-2 -mx-2 transition-colors">
                    {/* Icon per type */}
                    <div className={clsx(
                      "w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5",
                      ev.icon === 'new' ? "bg-blue-100 text-blue-600" :
                        ev.icon === 'picked' ? "bg-amber-100 text-amber-600" :
                          "bg-emerald-100 text-emerald-600"
                    )}>
                      {ev.icon === 'new' && <Ticket className="w-4 h-4" />}
                      {ev.icon === 'picked' && <Wrench className="w-4 h-4" />}
                      {ev.icon === 'done' && <CheckCircle2 className="w-4 h-4" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-[11px] font-semibold text-slate-500 leading-tight">
                        {ev.label} <span className="font-extrabold text-slate-800">{ev.actor}</span>
                      </p>
                      <p className="text-[12px] font-bold text-slate-700 truncate mt-0.5 group-hover:text-emerald-700 transition-colors">{ev.ticketTitle}</p>
                      <p className="text-[9px] font-semibold text-slate-400 mt-0.5">
                        {ev.ticketNumber} &bull; {new Date(ev.time).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })} {new Date(ev.time).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}
                      </p>
                    </div>
                  </Link>
                ))
      ...
      {/* ========================================================= */}
      {/* NOTIFICATION PANEL - Mobile: fullscreen | Desktop: right panel */}
      {/* ========================================================= */}
      {isNotifOpen && (
        <div className="fixed inset-0 z-[100] flex animate-in fade-in duration-200">
          ...
          {/* Panel — slides from right on all screens, but width differs */}
          <div className="relative ml-auto w-full md:w-[420px] h-full bg-slate-50 flex flex-col shadow-2xl animate-in slide-in-from-right-full duration-300">

            {/* Header */}
            <div className="px-4 py-4 flex items-center gap-3 bg-white shadow-sm z-10">
              <button
                onClick={() => setIsNotifOpen(false)}
                className="w-10 h-10 flex items-center justify-center rounded-full active:bg-slate-100 text-slate-700 transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
              <div className="flex-1">
                <h3 className="font-extrabold text-slate-800 text-lg leading-none">Notifikasi</h3>
                <p className="text-xs font-semibold text-slate-400 mt-0.5">{unreadCount > 0 ? `${unreadCount} belum dibaca` : 'Semua sudah dibaca'}</p>
              </div>
              {unreadCount > 0 && (
                <button
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
            {unreadCount > 0 && (
              <div className="flex justify-between items-center mb-4 px-1">
                ...
                <button
                  onClick={() => markAllAsRead()}
                  className="text-xs font-extrabold text-emerald-600 bg-emerald-50 px-3 py-1.5 rounded-full"
                >
                  Tandai Semua Dibaca
                </button>
              </div>
